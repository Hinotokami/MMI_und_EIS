...
class FrameW(qw.QWidget):
	...
	def fn(self, e):
		...
		if(e.key() == qc.Qt.Key_Left):
			self.direction = 0
		else if(e.key() == qc.Qt.Key_Right):
			self.direction = 1
		else if(e.key() == qc.Qt.Key_Up):
			self.direction = 2
		else if(e.key() == qc.Qt.Key_Down):
			self.direction = 3
	...
	def makePicture(self):
		...
		self.picture = qg.QImage(self.pictureWidth, self.pictureHeight, qg.QImage.Format_RGB32)
	...
	def makeMenu(self): #not done: buttons and positions
		...
		size,ok = qg.QInputDialog.getInt(self, 'field', 'width', value = self.pictureWidth / self.zoom, min = fieldMin, max = fieldMax, step = 1)
		if ok:
			self.pictureWidth = size * self.zoom
		size,ok = qg.QInputDialog.getInt(self, 'field', 'height', value = self.pictureHeight / self.zoom, min = fieldMin, max = fieldMax, step = 1)
		if ok:
			self.pictureHeight = size * self.zoom
		size,ok = qg.QInputDialog.getInt(self, 'field', 'zoom', value = self.zoom, min = zoomMin, max = zoomMax, step = 1)#pixel per field element
		if ok:
			self.zoom = size
		size,ok = qg.QInputDialog.getInt(self, 'snake', 'speed', value = self.speed, min = speedMin, max = speedMax, step = 1)#start speed
		if ok:
			self.speed = size
		size,ok = qg.QInputDialog.getInt(self, 'snake', 'speed', value = self.speedUp, min = speedUpMin, max = speedUpMax, step = 1)#per food eaten/amount before speedup
		if ok:
			self.speedUp = size
		size,ok = qg.QInputDialog.getInt(self, 'snake', 'food', value = self.food, min = foodMin, max = foodMax, step = 1)#food probability in percent
		if ok:
			self.food = size
	...
	def won(self): #not done: file manpipulation
		...
		#display 'message' in dialogBox with OK button and Show Highscore option -> show stuff (should still be saved)
		...
		if(scores.size() == 0):
			...
			#string split? get the points and names -> playerPoints[] -> playerNames[]
		...
		for i in range(0, rank.size()):
			string = "%d. %dp by %s" % (rank[i], playerPoints[i], playerNames[i])
		...
		if(position == 11):
			...
			highButton = wonMess.addButton(tr("View Highscores"), QMessageBox.ActionRole)
	...
	def moveGame(self): #not done: questions, optimization; have to try out!!
		...
		#borders on, meets: terminates game; optional: bounce? clockwise or counterclockwise? what else?
		...
		#propability for food to appear, random position that must be background
